...
ECIHand eciNew(void)
{
  ...
  if (!api_lock(api) && !process_func1(api, &header, NULL, &eci_res, false)) {
	if (eci_res != 0) {
...
Boolean eciSetOutputBuffer(ECIHand hEngine, int iSize, short *psBuffer)
{
  ...
  if (!api_lock(engine->api) && !process_func1(engine->api, &header, NULL, &eci_res, false)) {  
	if (eci_res == ECITrue) {
...
Boolean eciSetOutputFilename(ECIHand hEngine, const void *pFilename)
{
  ...
  if (!api_lock(engine->api) && !process_func1(engine->api, &header, pFilename, &eci_res, true)
      && (eci_res == ECITrue))
...
Boolean eciAddText(ECIHand hEngine, ECIInputText pText)
{
  ...
  if (!api_lock(engine->api))
    process_func1(engine->api, &header, pText, &eci_res, true);
  return eci_res;
}
...
Boolean eciSynthesize(ECIHand hEngine)
{
  ...
  if (!api_lock(engine->api))
    process_func1(engine->api, &header, NULL, &eci_res, true);
  return eci_res;
}
...
ECIHand eciDelete(ECIHand hEngine)
{
  ...
  if (!process_func1(engine->api, &header, NULL, (int*)&eci_res, false)) {
...
void eciRegisterCallback(ECIHand hEngine, ECICallback Callback, void *pData)
{
  ...
  if (!api_lock(engine->api) && !process_func1(engine->api, &header, NULL, NULL, false)) {
...
int eciSetParam(ECIHand hEngine, enum ECIParam Param, int iValue)
{
  ...
  if (!api_lock(engine->api) && !process_func1(engine->api, &header, NULL, &eci_res, true)) {  
...
int eciSetDefaultParam(enum ECIParam parameter, int value)
{
  ...
  if (!api_lock(api) && !process_func1(api, &header, NULL, &eci_res, true)) {  
...
int eciGetDefaultParam(enum ECIParam parameter)
{
  ...
  if (!api_lock(api))
    process_func1(api, &header, NULL, &eci_res, true);
...
void eciClearErrors(ECIHand hEngine)
{
  ...
  if (!api_lock(engine->api))
    process_func1(engine->api, &header, NULL, NULL, true);  
}
...
void eciVersion(char *pBuffer)
{
  ...
  *(char*)pBuffer=0;
  ...
  if (!api_lock(api) && !process_func1(api, &header, NULL, NULL, false)) {
...
ECIDictHand eciNewDict(ECIHand hEngine)
{
  ...
  if (!api_lock(engine->api))
    process_func1(engine->api, &header, NULL, (int*)&eci_res, true);
  return eci_res;
}
...
enum ECIDictError eciLoadDict(ECIHand hEngine, ECIDictHand hDict, enum ECIDictVolume DictVol, ECIInputText pFilename)
{
  ...
  if (!api_lock(engine->api))
    process_func1(engine->api, &header, pFilename, (int*)&eci_res, true);
  return eci_res;
}
