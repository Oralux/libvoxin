...
#include "debug.h"
  ...
  static int daemon_start(struct libvoxin_t *the_obj)
  {
	...
	case 0:
	  ...
	  pipe_close(the_obj->pipe_command, PIPE_SOCKET_PARENT_INDEX);
	  exit(child(the_obj));
  ...
  int libvoxin_create(libvoxin_handle_t *i)
  {
	...
	obj = (libvoxin_t *)calloc(1, sizeof(struct libvoxin_t));
	if (!obj)
	  return errno;

	res = pipe_create(&obj->pipe_command);
	if (res)    
	  goto exit0;

	obj->id = LIBVOXIN_ID;
	res = daemon_start(obj);
	if (!res)
	  *i = obj;
  
  exit0:
	if (res) {
	  pipe_delete(&obj->pipe_command);
	}
  ...
  int libvoxin_delete(libvoxin_handle_t *i)
  {
	...
	obj = (struct libvoxin_t*)*i;
  
	res = pipe_delete(&obj->pipe_command);
	if (res)    
	  goto exit0;
